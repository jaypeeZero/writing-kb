...
from writing_kb.content import safe_path as _safe_path, list_md_files as _list_md_files, read_file as _read_file, parse_sections
from writing_kb.search import SearchIndex
from writing_kb.tools import kb_list_topics, kb_read, kb_search


@pytest.fixture(autouse=True)
def reset_search_index():
    """Reset the search index between tests to prevent cross-test contamination."""
    search_module._search_index = search_module.SearchIndex()
    yield
    search_module._search_index = search_module.SearchIndex()
...
class TestListMdFiles:
    ...
    def test_returns_files_from_content_dirs(self, tmp_path, monkeypatch):
        monkeypatch.setattr(content_module, "KB_DIR", tmp_path)
        monkeypatch.setattr(content_module, "CONTENT_DIRS", {"craft"})
    ...
    def test_missing_content_dir_is_skipped(self, tmp_path, monkeypatch):
        monkeypatch.setattr(content_module, "KB_DIR", tmp_path)
        monkeypatch.setattr(content_module, "CONTENT_DIRS", {"craft", "style"})
        (tmp_path / "craft").mkdir()
        (tmp_path / "craft" / "dialogue.md").write_text("content")
...
class TestReadFile:
    ...
    def test_blocks_traversal_outside_kb_dir(self, tmp_path, monkeypatch):
        monkeypatch.setattr(content_module, "KB_DIR", tmp_path / "kb")
        monkeypatch.setattr(content_module, "CONTENT_DIRS", {"craft"})
        (tmp_path / "kb").mkdir()
...
class TestKbListTopics:
    ...
    async def test_with_path_returns_section_headers(self):
        ...
        with patch("writing_kb.tools.read_file", return_value=content):
    ...
    async def test_with_path_handles_404(self):
        with patch("writing_kb.tools.read_file", return_value=None):
...
class TestKbRead:
    ...
    async def test_adds_md_extension(self):
        with patch("writing_kb.tools.read_file", return_value="# Content") as mock_read:
...
class TestKbSearch:
    ...
    def _load_index(self, *section_args):
        search_module._search_index.build(make_sections(*section_args))
    ...
    async def test_empty_index_returns_no_results(self):
        search_module._search_index.build([])
